...
class DashboardRecognition:
    def __init__(self, min_area=50 * 50):
        ...
        self.last_bbox = None
        self.last_dashboard_status = None
        ...
        if self.bbox is not None:
            ...
            if max_contour is not None:
                ...
                if 0 <= score <= 0.3:
                    ...
                    self.dashboard_status = self.last_dashboard_status
                    
                self.last_dashboard_status = self.dashboard_status
                return self.dashboard_status

        return self.last_dashboard_status
        ...
        if self.bbox is not None:
            ...
            if self.dashboard_status is not None:
                cv.putText(image, self.dashboard_status, (self.bbox[0][0], self.bbox[0][1] + 22 * thickness),
                           cv.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), thickness)
        return image
...
if __name__ == '__main__':
    ...
    while True:
        ...
        cv.imshow("dashboard detection", frame)
        k = cv.waitKey(1)
        if k == 113 or k == 81:  # q or Q to quit
            cap.release()
            cv.destroyWindow("dashboard detection")
            break
