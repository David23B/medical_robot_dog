...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not ret:
            continue
        bbox = dashboard_detector.detect(img)
        img = dashboard_detector.visualize(img)
        status = dashboard_detector.get_status(img)
        
        if bbox is not None:
            match_result = np.array([[bbox[0][0], bbox[0][1], bbox[1][0], bbox[1][1], 1]])
            ...
            for tracker in trackers:
                ...
                cv.rectangle(img, (int(tracker[0]), int(tracker[1])), (int(tracker[2]), int(tracker[3])), color_tracker, 2)
        ...
        cv.imshow("Danger Sign Recognition", img)
        k = cv.waitKey(1)
